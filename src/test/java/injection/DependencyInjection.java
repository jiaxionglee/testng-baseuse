...
public class DependencyInjection {
    ...
    public void afMethod(ITestResult iTestResult) {
        System.out.println("@AfterMethod-方法名称：" + iTestResult.getMethod().getMethodName()
                + " 执行状态（1-成功，2-失败，3-skip）：" + iTestResult.getStatus());
    }
    ...
    public void afClass(ITestContext iTestContext) {
        ...
        for (ITestResult iTestResultPass : iResultMapSetPass) {
            System.out.println("@AfterClass-" + iTestResultPass.getMethod().getMethodName()
                    + " 方法执行状态（1-成功，2-失败，3-skip）：" + iTestResultPass.getStatus());
        }
        ...
        for (ITestResult iTestResultFail : iResultMapSetFail) {
            System.out.println("@AfterClass-" + iTestResultFail.getMethod().getMethodName()
                    + " 方法执行状态（1-成功，2-失败，3-skip）：" + iTestResultFail.getStatus());
        }
    }
}
